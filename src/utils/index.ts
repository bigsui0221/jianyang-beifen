 ...
 * @param {Function} func
 * @param {number} wait
 * @param {boolean} immediate 防抖模式（immediate = false）节流模式（immediate = true）
 * @return {*}
 */
export function debounceOrThrottle<T extends (...args: any[]) => any>(
  func: T,
  wait: number,
  ...
  const later = function (this: typeof context) {
    // 计算上次触发时间间隔
    ...
    if (last < wait && last > 0) {
      ...
      if (!immediate) {
        ...
        if (!timeout) {
          context = null as unknown as ThisParameterType<T>;
